...
public class RASRsdk {
	private static String secret_key, secretid, appid, engine_model_type, res_type, result_text_format, voice_format,
	...
	public static int setConfig(String secret_key, String secretid, String appid, String engine_model_type,
			String res_type, String result_text_format, String voice_format, String filepath, int cutlength) {
		return RASRsdk.setConfig(secret_key, secretid, appid, engine_model_type, res_type, result_text_format,
				voice_format, filepath, cutlength, "");
	}
	...
	public static int setConfig(String secret_key, String secretid, String appid, String engine_model_type,
		...
		if (engine_model_type.length() <= 0
				|| (engine_model_type.compareTo("8k_0") != 0 && engine_model_type.compareTo("16k_0") != 0)) {
		...
		if (result_text_format.length() <= 0
				|| (result_text_format.compareTo("0") != 0 && result_text_format.compareTo("1") != 0
						&& result_text_format.compareTo("2") != 0 && result_text_format.compareTo("3") != 0)) {
			System.out.println("result_text_format is not vailed!");
			return -1;
		}
		if (voice_format.length() <= 0 || (voice_format.compareTo("1") != 0 && voice_format.compareTo("4") != 0
				&& voice_format.compareTo("6") != 0)) {
	...
	public static int sendVoice() {
		...
		Map<String, String> Str = new TreeMap();
		...
		// sub_service_type是子服务类型，0：离线语音识别；1：实时流式识别
		reqMap.put("sub_service_type", "1");
		// 引擎类型8k_0:8k通用；16k_0:16k通用
		reqMap.put("engine_model_type", engine_model_type);
		// 返回方式，0：同步返回；1：异步返回，目前只支持异步
		reqMap.put("res_type", res_type);
		// 识别结果的编码方式，0：UTF-8；1：GB2312:；2：GBK；3：BIG5
		...
		reqMap.put("nonce", getNonce());// 随机正整数
			...
			while (true) {
				...
				if (n == cutlength || (n < cutlength && end == 0)) {
					...
					try {
						...
						System.out.println(jsObj.get("text"));
						//最后一段写入文件
						if (end == 1) {
							FileOutputStream fw = new FileOutputStream("output.txt");
							fw.write(jsObj.get("text").toString().getBytes());
							fw.flush();
							fw.close();
						}
